...
class Data_trans(object):
	...
	def load_sem_data(self,dataset,split):
		...
		print('load:',split,'->',len(texts))
		return texts,labels

	# load the train, valid or test
	def load_masks(self,dataset,splits):
		...
		res = []
		for tokens_list in texts_list_train_test:
			masks = self.role_mask.get_masks(tokens_list,None,self.opt.max_sequence_length, self.opt.all_roles)
			res.append(masks)
		return res
...
if __name__ == '__main__':
	...
	data_help = Data_trans(args)
	splits = ['train','val']
	print('going to start:',splits)
	train_mask,test_mask = data_help.load_masks('wmt16', splits)
	print(train_mask[:5])
	print('-'*10)
	print(test_mask[:5])
