...
class ScaledDotProductAttention_orig():
...
# original multiattention
class MultiHeadAttention_orig():
	...
	def __init__(self, n_head, d_model, dropout, mode=0):
		...
		self.attention = ScaledDotProductAttention_orig()
...
class DecoderLayer():
	def __init__(self, d_model, d_inner_hid, n_head, dropout=0.1):
		self.self_att_layer = MultiHeadAttention_orig(n_head, d_model, dropout=dropout)
		self.enc_att_layer  = MultiHeadAttention_orig(n_head, d_model, dropout=dropout)
		self.pos_ffn_layer  = PositionwiseFeedForward(d_model, d_inner_hid, dropout=dropout)
...
class MultiLayerEncoder():
	...
	def __call__(self, src_emb, src_seq, return_att=False, active_layers=999, masks = None):
		...
		if masks == None:
			for i in range(self.n_head): all_masks.append(pad_mask)	
		else:
...
# SelfAttention(d_model, d_inner_hid, n_head, layers, dropout); self.encoder(src_emb, src_seq, active_layers=active_layers)
