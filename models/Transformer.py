...
class Transformer(BasicModel):
	def get_model(self,opt,embedding_type='word', active_layers=999) :   
...
#		i_word_emb = Embedding(opt.max_sequence_length, d_emb)
		...
		if embedding_type=='dep':
			...
			encoder = SelfAttention(opt.dep_dim, opt.d_inner_hid, opt.n_head, opt.layers, opt.dropout_rate)
			pos_emb = PosEncodingLayer(opt.max_sequence_length, opt.dep_dim) # if self.src_loc_info else None
